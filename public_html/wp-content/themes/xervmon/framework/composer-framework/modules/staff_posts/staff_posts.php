...
if(is_admin()){
	...
	$module[] = array( 	'title' => 'Item Size',
						...
						'std'   => 'one_third',
						'type'  => 'select',
						'opts'	=> array( '1/1' => 'one_one', '1/2' => 'one_half', '1/3' => 'one_third', '1/4' => 'one_fourth' ) );
		...
		if ($jw_query->have_posts()) : while ($jw_query->have_posts()) : $jw_query->the_post(); /* Loop the posts */ $count++; $real_count++;
			...
			$output .= '<article class="staff-post-entry '.$size_class.$last.$clear.$margin_class.'">';
				...
				if($staff_show_pictures == 'yes'){
					...
					$output .= '<span class="staff-post-thumbnail" style="float:left;">';
					
						$output .= get_the_post_thumbnail(get_the_ID(), $thumbnail_size, 'style=height:200px !important;width:170px !important;');
						
					$output .= '</span><!-- .staff-post-thumbnail -->';
				
				}
				
				if($staff_show_names == 'yes'){
				
					$output .= '<div style="margin: 0 0 0 230px;"><h1 class="staff-post-title">'.$post_options['jw_staff_member_name'].'</h1>';
