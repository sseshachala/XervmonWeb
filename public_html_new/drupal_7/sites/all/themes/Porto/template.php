...
function porto_form_alter(&$form, &$form_state, $form_id) {
 ...
 if($form['#id'] == 'user-register-form'){
    $form['account']['mail']['#description'] = t(''); //Removing description from email field.
    $form['account']['name']['#description'] = t(''); //Removing description from password field.
  }
...
function porto_menu_alter(&$items) {
...
function porto_theme() {
    return array(
        'signup_signupform' => array(
            'render element' => 'form',
            'template' => 'signup_signupform',
            'path' => drupal_get_path('theme', 'porto') . '/templates',
        )    
    );
}

function porto_preprocess_signup_signupform(&$vars) {
     ...
     $vars['first_name'] = drupal_render($vars['form']['first_name']);
     $vars['last_name'] = drupal_render($vars['form']['last_name']);
     $vars['email'] = drupal_render($vars['form']['email']);
     $vars['phone'] = drupal_render($vars['form']['phone']);
